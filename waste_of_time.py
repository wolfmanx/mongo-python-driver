...
def make_key(data):
    key_indx = random.randint(0, len(data) - 1)
...
def report_times(total_time, wasted_time=None, totally_wasted_time=None, ref_time=None):
    now = datetime.now()
    tzero = now - now
    hl_ok = False
    if wasted_time is not None:
        ...
        print('wasted_time_sum (' + str(CYCLES) + '/' + str(CYCLES * DATA_COUNT) + ')        : ' + str(wasted_time_sum))
        print('wasted_time_avg (' + str(CYCLES) + '/' + str(CYCLES * DATA_COUNT) + ')        : ' + str(wasted_time_avg))
    ...
    if totally_wasted_time is not None:
        ...
        print('totally_wasted_time_sum (' + str(CYCLES) + '/' + str(CYCLES * DATA_COUNT) + '): ' + str(totally_wasted_time_sum))
        print('totally_wasted_time_avg (' + str(CYCLES) + '/' + str(CYCLES * DATA_COUNT) + '): ' + str(totally_wasted_time_avg))
        ...
        if wasted_time is not None:
            ...
            print('overall_wasted_time_sum (' + str(CYCLES) + '/' + str(CYCLES * DATA_COUNT) + '): ' + str(overall_wasted_time_sum))
            print('overall_wasted_time_avg (' + str(CYCLES) + '/' + str(CYCLES * DATA_COUNT) + '): ' + str(overall_wasted_time_avg))

    if total_time is not None:
        if hl_ok:
            hl()
        if ref_time is not None:
            factor = ' factor: ' + str((total_time.seconds * 1000000 + total_time.microseconds) / (ref_time.seconds * 1000000 + ref_time.microseconds))
        else:
            factor = ''
        print('total_time (' + str(CYCLES) + '/' + str(CYCLES * DATA_COUNT) + ')             : ' + str(total_time) + factor)
...
def check_type_with_json_then_bson(key, val, dcopy, wt, totally_wasted_time):
    start = datetime.now()
    try:
        ...
        try:
            ...
            for key, val in state.items():
                check_type_with_json_then_bson(key, val, state, wt, totally_wasted_time)
...
def waste_some_space_and_time_converting_data_with_json_bson_default(ref_time):
    ...
    for indx in range(CYCLES):
        dcopy = dict()
        key, key_indx = KEYS[indx]
        data[key] = CONVERTIBLE()
        wt = wasted_time
        for key, val in data.items():
            wt = check_type_with_json_then_bson(key, val, dcopy, wt, totally_wasted_time)
        data[key] = key_indx
...
def dont_waste_time_with_getstate_feature(ref_time):
    # works always

    try:
        sv_getstate_enabled = bson.is_getstate_enabled()
    except AttributeError:
        printe('SKIPPED')
        return
    ...
    for indx in range(CYCLES):
        ...
        # encode with getstate
        bson.BSON.encode(data)

        data[key] = key_indx


    bson.enable_getstate(sv_getstate_enabled)
    total_end = datetime.now()
    total_time = total_end-total_start
...
def test_run():                                            # ||:fnc:||
    ...
    try:
        hl('dont_waste_time_with_getstate_feature()')
        bson.enable_c(True)
        dont_waste_time_with_getstate_feature(ref_time)
    except:
        (t, e, tb) = sys.exc_info()
        printe(''.join(traceback.format_tb(tb)))
        printf(sformat('{0}: {1}', t.__name__, e))
...
def run(parameters, pass_opts):                            # ||:fnc:||
    ...
    hl_lvl(1)
    hl('CONVERTIBLE=FlatConvertible')
    hl_lvl(0)
    CONVERTIBLE=FlatConvertible
    ...
    hl_lvl(1)
    hl('CONVERTIBLE=DeepConvertible')
    hl_lvl(0)
