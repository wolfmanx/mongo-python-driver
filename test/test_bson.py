    ...
    def test_enable_c(self):
        ...
        def check_c_state(self, encoding, decoding):
            ...
            if not (encoding or decoding):
                ...
                if encoding:
                    self.assertTrue(bson._ctx._use_c_encoding)
                    self.assertTrue(repr(bson._ctx._dict_to_bson).startswith('<built-in function'))
                else:
                    self.assertFalse(bson._ctx._use_c_encoding)
                    self.assertTrue(repr(bson._ctx._dict_to_bson).startswith('<function'))

                if decoding:
                    self.assertTrue(bson._ctx._use_c_decoding)
                    self.assertTrue(repr(bson._ctx._bson_to_dict).startswith('<built-in function'))
                    self.assertTrue(repr(bson._ctx._decode_all).startswith('<built-in function'))
                else:
                    self.assertFalse(bson._ctx._use_c_decoding)
                    self.assertTrue(repr(bson._ctx._bson_to_dict).startswith('<function'))
                    self.assertTrue(repr(bson._ctx._decode_all).startswith('<function'))
